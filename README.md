...
This document contains a brief description of the project layout and instructions for building and running the *clickstreamgen* data generator. Users that are not familiar with the build and setup specifics of Myriad based data generators should read the following text carefully before they start using the downloaded software.
...
comprising the *clickstreamgen* data generator as *clickstreamgen extensions* or *generator extensions*.
...
The parallel deployment script generated under `bin/clickstreamgen-frontend` as well as some other utility scripts are written in Python. For those to work, you have to make sure that the build host and the master node in your distributed build environment (normally the same machine) have Python >= 2.4.3 installed.
...
This should build and install the data generator under `<install-path>/clickstreamgen`. For the remainder of this document, we will use `<clickstreamgen-install>` as an alias for `<install-path>/clickstreamgen`.


Manually Starting Generator Nodes
=================================

If you look at the contents of the `<clickstreamgen-install>/bin` folder, you will find two files -- `clickstreamgen-frontend` and `clickstreamgen-node`. 
...
Once the values for these parameters are specified, you can start node `<i>` of your `<N>`-node parallel generation setup manually by typing:
    ...
    <clickstreamgen-install>$ bin/clickstreamgen-node -m<DATASET_ID> -s<SF> -i<i> -N<N> -o<PATH>
...
If you want to use the Python parallel deployment frontend, you have to setup your parallel environment. For this purpose, you need to create a `clickstreamgen-frontend.xml` configuration inside the `<clickstreamgen-install>/config` folder (you can use `clickstreamgen-config.template.xml` bundled during the build process as a basis). The Python frontend config files has the following structure:
...
For instance, consider a scenario where you have a 10 node cluster but only want to use the first five nodes for the data generation process. This will translate into the following `<cloud-env>` entry:
    ...
    <cloud-env id="mycloud-05" dgen="<clickstreamgen-install>">
      <master>
...
Now suppose that at a later point you want to use all 10 nodes. You can add a second `<cloud-env>` to reflect this: 
    ...
    <cloud-env id="mycloud-10" dgen="<clickstreamgen-install>">
      <master>
        <host name="myhost01.example.com" />
      </master>
      <slaves>
...
* The `<clickstreamgen-install>` should be is accessible from all nodes (this is best achieved through NFS exported install path).
...
We recommend choosing a `nodes-per-host` value in the range [X,2X] where X is the number of processor cores in each host in the referenced environment. You can start the parallel deployment script for a particular <DATASET_ID> and scaling factor <SF> with this command:
    ...
    <clickstreamgen-install>$ bin/clickstreamgen-frontend -m<DATASET_ID> -s<SF> <DGEN-CONFIG-ID>
