...
inline const string& Session::hashUID() const
{
	if (_hash_uid.empty())
	{
		MD5Engine engine;
		engine.update(toString(genID()));

		const_cast<Session*>(this)->_hash_uid = DigestEngine::digestToHex(engine.digest());
	}
